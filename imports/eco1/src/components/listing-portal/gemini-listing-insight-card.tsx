...
interface GeminiListingInsightCardProps {
    insight: string;
    actionLink: string;
    actionText: string;
}

export function GeminiListingInsightCard({ insight, actionLink, actionText }: GeminiListingInsightCardProps) {
    return (
        <Card className="col-span-full bg-gradient-to-br from-primary/10 to-accent/10 border-primary/20 shadow-lg">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium flex items-center gap-2">
                    <Bot className="h-4 w-4 text-primary" /> Gemini Listing Insight
                </CardTitle>
